...
{   
	public:
		...
		void InsertElem(DataType data);//插入元素
...
//在指定位置插入指定元素
void LinkList::InsertElemAtIndex(DataType data,int n)//n是指定位置
{  
...
void LinkList::InsertElem(DataType data)
{
	Node * p=head;//新建一个指针指向头节点
	Node * temp=new Node;//新建一个指针，指向传进来的data节点
	temp->data=data;
	temp->next=NULL;
	if(IsEmpty())
	{
